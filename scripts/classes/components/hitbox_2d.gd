@icon("res://assets/components/Hitbox2DNode.svg")
class_name Hitbox2D
extends Area2D
## Deals damage to any [Hurtbox2D] it overlaps.
##
## Attach to weapons, projectiles, or hazard volumes. Configure collision layers/masks
## in the editor so this only detects the appropriate [Hurtbox2D] nodes.
##
## @experimental

signal hit_landed(hurtbox : Hurtbox2D, damage_amount : float)
signal hit_blocked(hurtbox : Hurtbox2D)
...
## Optional reference to the attacking entity, forwarded to [Hurtbox2D] for attribution
## (e.g. kill credit, damage logs).
@export var source : Node

@export_category("Repeat Hit Settings")
## Seconds before this hitbox can hit the same [Hurtbox2D] again while still overlapping.
...
## Clears all recorded hit-cooldown timestamps, allowing every overlapping [Hurtbox2D]
