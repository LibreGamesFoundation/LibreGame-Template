@icon("res://assets/components/Hurtbox3DNode.svg")
class_name Hurtbox3D
extends Area3D
## Receives hits from any [Hitbox3D] it overlaps and forwards damage to [member target].
##
## Automatically added to the "hurtbox" group so [Hitbox3D] can detect it without a
## direct type dependency. Configure collision layers/masks in the editor.
##
## @experimental

signal hit_received(hitbox : Hitbox3D, damage_amount : float, damage_type : Hitbox3D.DamageType)
...
## Called by [Hitbox3D] when it overlaps this hurtbox. Returns true if the hit was
## accepted, or false if it was ignored (e.g. due to invincibility).
func receive_hit(hitbox : Hitbox3D) -> bool:
	if invincible:
